...
class SigningPipeline(private val context: Context) {
    ...
    suspend fun signAdifFile(
        ...
        val certificate = certManager.getCertificate(certAlias)
            ?: error("Certificate not found.")

        // Match TrustedQSL's fundamental certificate constraints before signing.
        // LoTW will reject QSOs outside the certificate's QSO validity window.
        val certQsoNotBefore = certManager.parseCertInfo(certificate).qsoNotBefore
        val certQsoNotAfter = certManager.parseCertInfo(certificate).qsoNotAfter
        ...
        for ((index, rawFields) in qsoMaps.withIndex()) {
            ...
            // If station-location fields are present in the ADIF, they must
            // be consistent with the selected station. A station may represent
            // one 4/6-character grid or two/four adjacent 4-character VUCC grids.
            val stationGrids = station.grid.split(",")
                .map { it.trim().uppercase() }
                .filter { it.isNotEmpty() }
            val myGrid = rawFields["MY_GRIDSQUARE"]?.trim()?.uppercase().orEmpty()
            val myVuccGrids = rawFields["MY_VUCC_GRIDS"]?.trim()?.uppercase()
                ?.split(",")
                ?.map { it.trim() }
                ?.filter { it.isNotEmpty() }
                ?: emptyList()

            if (stationGrids.isNotEmpty()) {
                val stationPrefixes = stationGrids.map { it.take(4) }.toSet()

                val myGridMatches = myGrid.isEmpty() ||
                    stationPrefixes.contains(myGrid.take(4))
                val myVuccMatches = myVuccGrids.isEmpty() ||
                    myVuccGrids.all { stationPrefixes.contains(it.take(4)) }

                if (!myGridMatches || !myVuccMatches) {
                    gridMismatchCount++
                    continue
                }
            }
            ...
            // The ADIF station callsign, when present, must match the selected
            // callsign certificate/station location.
            val stationCall = rawFields["STATION_CALLSIGN"]?.trim()?.uppercase()
                ?: rawFields["OPERATOR"]?.trim()?.uppercase()
            if (!stationCall.isNullOrEmpty() && stationCall != station.callSign.uppercase()) {
                invalidCount++
                continue
            }

            // SAT QSOs require PROP_MODE=SAT and SAT_NAME, as documented by LoTW.
            if (qso.propMode == "SAT" && qso.satName.isBlank()) {
                invalidCount++
                continue
            }

            if (certQsoNotBefore != null && qso.date.isBefore(certQsoNotBefore)) {
                dateFilterCount++
                continue
            }
            if (certQsoNotAfter != null && qso.date.isAfter(certQsoNotAfter)) {
                dateFilterCount++
                continue
            }
            ...
            // Fail locally rather than producing a TQ8 that cannot be verified.
            val verifier = Signature.getInstance("SHA1withRSA")
            verifier.initVerify(certificate.publicKey)
            verifier.update(signdata.toByteArray(Charsets.US_ASCII))
            check(verifier.verify(sigBytes)) { "Internal LoTW signature verification failed" }
    ...
    suspend fun signQsoList(
        ...
        for ((index, qso) in qsos.withIndex()) {
            ...
            // Keep the in-memory editor path subject to the same station-location
            // consistency checks as imported ADIF logs.
            ...
            if (certQsoNotBefore != null && qso.date.isBefore(certQsoNotBefore) ||
                certQsoNotAfter != null && qso.date.isAfter(certQsoNotAfter)) {
                dateFilterCount++
                continue
            }
